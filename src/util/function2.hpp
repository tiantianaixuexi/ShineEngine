...
// 让条件成立  static_assert(std::is_same_v<typename deduce_to_void<>::type, void>);
template <typename...>
struct deduce_to_void : std::common_type<void> {};

// 任意类型，不管是多少参数，都成立上面的条件
template <typename... T>
using void_t = typename deduce_to_void<T...>::type;

// 移除类型 T上所有顶层的引用（reference）以及 const 和 volatile 限定符（cv-qualifiers）​​，得到一个“纯粹”的基础类型
template <typename T>
