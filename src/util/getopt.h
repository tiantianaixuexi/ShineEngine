...
{
    inline std::string WString_to_String__(const std::wstring& wstr)
    {
        ...
        WideCharToMultiByte(CP_UTF8, 0, wstr.c_str(), static_cast<int>(wstr.size()), buffer, len, nullptr, nullptr);
...
#       ifdef _WIN32
        ...
        if (list) {
            for (int i = 0; i < argv; ++i) {
                ...
                args.push_back(WString_to_String__(ws));
            }
            LocalFree(list);
        }
#       else
...
struct getopt : public std::map< std::string, std::string >
{
    ...
    getopt(const std::vector<std::string>& args) : super() {
        ...
        *this = getopt(static_cast<int>(argv.size()), argv.data());
    }
