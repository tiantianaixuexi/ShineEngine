...
void AssetsBrower::onInit() {
    ...
    if (EngineContext::IsInitialized()) {
        ...
        // 注册图标缓存搜索路径由 IconCache::Init 负责，此处仅取系统指针
        iconCache_ = ctx.GetSystem<shine::editor::IconCache>();
    }
...
// -----------------------------------------------------------------------
//  GetAssetTypeIconKey — 资产类型 → 图标名查找表（零 if-else）
// -----------------------------------------------------------------------
const char* AssetsBrower::GetAssetTypeIconKey(STextView type) noexcept
{
    static constexpr struct {STextView type; const char* icon; } kTypeIcons[] = {
        // 顶层资产类型
        {"model",           "icon_asset_model"},
        {"texture",         "icon_asset_texture"},
        {"material",        "icon_asset_material"},
        {"audio",           "icon_asset_audio"},
        {"animation",       "icon_asset_animation"},
        {"script",          "icon_asset_script"},
        {"world",           "icon_asset_world"},
        {"blueprint",       "icon_asset_blueprint"},
        {"render_pipeline", "icon_asset_render_pipeline"},
        {"font",            "icon_asset_font"},
        // 子资产类型
        {"mesh",            "icon_asset_mesh"},
        {"skeleton",        "icon_asset_skeleton"},
        {"morph",           "icon_asset_morph"}, 
    };
    for (const auto& e : kTypeIcons)
        if (e.type == type)
            return e.icon;
    return "icon_asset_default";
}
...
void AssetsBrower::UpdateLayoutSizes(float availWidth, int itemCount) {
    ...
    layoutColumnCount_ = shine::math::Max(1, static_cast<int>(aw / (rawItemW + spacing)));
    layoutLineCount_   = (itemCount + layoutColumnCount_ - 1) / layoutColumnCount_;

    if (stretchSpacing_ && layoutColumnCount_ > 1)
        spacing = floorf(aw - rawItemW * static_cast<float>(layoutColumnCount_)) / static_cast<float>(layoutColumnCount_);
    ...
    layoutSelectableSpacing_ = shine::math::Max(floorf(spacing) - static_cast<float>(iconHitSpacing_), 0.0f);
...
void AssetsBrower::RenderAssetGrid() {
    // 1. 按需扫描并过滤当前目录
    // 只在目录切换、搜索词变化或 gridDirty_ 时重新扫描，避免每帧遍历文件系统
    // 只显示：子目录 + .sasset 文件；原始源文件（.png/.glb 等）不在网格展示
    const bool dirChanged    = (selectedDirectory_ != cachedGridDirectory_);
    const bool filterChanged = (std::memcmp(searchBuffer_, cachedSearchFilter_, sizeof(searchBuffer_)) != 0);
    if (gridDirty_ || dirChanged || filterChanged) {
        gridEntries_.clear();
        ...
        cachedGridDirectory_ = selectedDirectory_;
        std::memcpy(cachedSearchFilter_, searchBuffer_, sizeof(cachedSearchFilter_));
        gridDirty_ = false;
    }
    ...
    while (clipper.Step()) {
        for (int lineIdx = clipper.DisplayStart; lineIdx < clipper.DisplayEnd; ++lineIdx) {
            ...
            const int itemMax = shine::math::Min(itemMin + layoutColumnCount_, itemCount);
    ...
    if (ImGui::IsWindowHovered() && io.MouseWheel != 0.0f &&
        ...
        if (fabsf(zoomWheelAccum_) >= 1.0f) {
            ...
            const int   hoverIdx = static_cast<int>(hoverNy) * layoutColumnCount_ + static_cast<int>(hoverNx);

            iconSize_ *= powf(1.1f, static_cast<float>(static_cast<int>(zoomWheelAccum_)));
            iconSize_ = shine::math::Clamp(iconSize_, 16.0f, 128.0f);
