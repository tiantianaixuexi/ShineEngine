...
namespace shine { namespace wasm {
...
struct TexLoader {
  ...
  inline int request_url(int ctxId, const char* url, int urlLen, int* out_texId, int* out_w, int* out_h) {
    if (!url || urlLen <= 0 || !out_texId) return 0;
    const int cached = js_tex_load_url_sync(ctxId, shine::wasm::ptr_i32(url), urlLen);
    if (cached != 0) {
      *out_texId = cached;
    ...
    js_tex_load_url(ctxId, shine::wasm::ptr_i32(url), urlLen, id);
    return id;
  }

  inline int request_dataurl(int ctxId, const char* data, int dataLen, int* out_texId, int* out_w, int* out_h) {
    if (!data || dataLen <= 0 || !out_texId) return 0;
    const int cached = js_tex_load_dataurl_sync(ctxId, shine::wasm::ptr_i32(data), dataLen);
    if (cached != 0) {
      *out_texId = cached;
      set_wh_from_tex(ctxId, cached, out_w, out_h);
      return 0;
  ...
  inline int request_base64(int ctxId, const char* mime, int mimeLen, const char* b64, int b64Len,
